# Product Roadmap — AI 語音對話式 CRM (MVP 階段計畫)

**版本**：v2026-04-13（最後更新）  
**狀態**：現行文件 — Phase 1~4 詳細執行狀態  
**關聯文件**：長期路線圖見 `roadmap_full.md`；UI 規格見 `01_features/liff-app/` 與 `01_features/web-app-crm/`

> 專注於打通「實體名片數位化」與「業務拜訪紀錄」的斷點，用最短時間驗證產品核心價值。

---

## 產品願景 (Product Vision)

傳統 CRM 停留在「資料庫」概念，高度依賴業務員手動輸入（hand-key），導致資訊落後且難以發揮實際效用。
我們的全新願景是打造一個**無縫且流暢的 AI Agent**，讓業務員在外出的各種社交與拜訪後，只需透過「名片掃描 + AI 語音輸入」進行口語化的 Recap。AI 會自動解析語意、將資訊結構化建檔、更新專案進度，並主動推播針對該客戶的 **Next Step**。
這不再只是一個在 LINE 上的名片機器人，而是一個**以 Action-Driven 為核心、跨越手機與桌面的全方位對話式 CRM**。

> **本文件聚焦於 MVP 階段 (Phase 1 ~ Phase 4)。中長期完整願景請見 `產品開發路線圖.md`。**

---

## 設計原則

1. **極速輸入體驗優先 (Speed to Data)**：融合大相機按鈕 (FAB)、語音錄音與流暢的 AI 處理進度條，降低業務紀錄阻力。
2. **LINE / LIFF 優先，但不綁定**：以 LINE 作為免下載 App 的第一入口，結合 LIFF 打造趨近原生 App 的流暢體驗。
3. **平台解耦與技術收斂**：堅守 FastAPI + Firebase + Gemini 架構。所有外部串接由後端 Service Layer 負責，保持前端輕量。
...
## Phase 4 — Conversational CRM (MVP 最終階段)
...
## Phase 5 — EDM 個人化電子報（POC）

**狀態**：規劃中（2026-04-21 設計完成）

> 針對「口袋名單」場景：業務員用自己的個人 Gmail 帳號，對 CRM contacts 進行差異化的個人化電子報寄送，無需依賴公司 CRM。

**背景**：以 Avnet FAE 客戶需求為 POC 切入，驗證後可推廣至其他有同類需求的業務員。

### 功能清單

- [ ] **Gmail OAuth 連結**：用戶在 Admin App 授權個人 Gmail，refresh token 存 Firebase
- [ ] **分眾管理（Segments）**：依 contact tags 篩選 + 手動加入/剔除，儲存規則非快照
- [ ] **個人化模板**：HTML 貼入，支援 `{{name}}`、`{{company}}`、`{{opening_line}}` merge tags
- [ ] **`{{opening_line}}` AI 生成**：根據 contact 最新 activity 的 `ai_key_insights`，由 Gemini 生成 30 字開場白
- [ ] **逐人發信**：每位收件人獨立一封，`Cc` 公司信箱，走 Gmail API（非 SMTP）
- [ ] **Cloud Tasks 非同步執行**：後台寄送，完成後 Admin App 顯示結果
- [ ] **寄送記錄**：歷史寄送清單（日期、主旨、分眾名稱、人數、狀態）
...
EDM_CC_DEFAULT_EMAIL
```

### 驗收標準

- FAE 可在 Admin App 連結個人 Gmail（OAuth 完成）
- 分眾可依標籤篩選 + 手動調整，預覽符合人數
- 50 人名單寄送：全數成功送達，每封收件人不同、開場白個人化
- 公司信箱自動 CC、寄件人為個人 Gmail

### 設計文件

`docs/superpowers/specs/2026-04-21-edm-newsletter-design.md`
...
| Phase 2 Pilot | 修復 Onboarding、CSV 匯出 | 完成 onboarding、內部團隊試用回饋 |
| Phase 3 體驗升級 | 系統 LIFF 化、流暢 UX 導入 | LIFF 編輯體驗流暢、讀取無卡頓 |
| Phase 4 Conversational CRM | 語音/意識流建檔、NLU 解析 | 端對端業務情境主流程順暢運作 |
...
| Phase 5 EDM 個人化電子報 | Gmail OAuth + 分眾管理 + AI 個人化寄信 | FAE POC 驗證成功，50 人名單端對端通過 |
| Phase 6 串接與自動化 | 外部 CRM 串接、進階分析 | 由真實用戶行為數據驅動 |